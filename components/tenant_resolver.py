...
# ) -> str:
#     # 1. explicit header is strongest
#     if x_tenant_id:
#         slug = x_tenant_id.strip().lower()
#         if slug in TENANTS:
#             return slug

#     # 2. from path like /client/futuretech
...
#     # 3. from host / origin / referer
#     candidates = []

#     host = _clean_host(request.headers.get("host"))
#     origin = _clean_host(urlparse(request.headers.get("origin", "")).netloc) if request.headers.get("origin") else None
#     referer = _clean_host(urlparse(request.headers.get("referer", "")).netloc) if request.headers.get("referer") else None
